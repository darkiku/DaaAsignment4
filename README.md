...
Java Development Kit (JDK) 11 or higher - download from adoptium.net
Apache Maven 3.6 or higher - download from maven.apache.org
Git (optional) - for cloning repository from git-scm.com

Verify installation:
bashjava -version    # Should show Java 11+
...
bashmkdir -p data/small
mkdir -p data/medium
mkdir -p data/large
This creates the directory structure where generated datasets will be stored.
9.3 Building the Project
...
bashmvn package
This creates target/graph-algorithms-1.0-SNAPSHOT.jar containing all compiled classes.
9.4 Generating Test Datasets
...
Small datasets:
  ✓ data/small/small_1.json - 6 nodes, sparse, with cycles (edges: 6)
  ✓ data/small/small_2.json - 8 nodes, pure DAG (edges: 11)
  ✓ data/small/small_3.json - 10 nodes, sparse, with cycles (edges: 11)

Medium datasets:
  ✓ data/medium/medium_1.json - 12 nodes, 3 SCCs of size 4 (edges: 20)
  ✓ data/medium/medium_2.json - 15 nodes, mixed structure (edges: 23)
  ✓ data/medium/medium_3.json - 18 nodes, dense DAG (edges: 50)

Large datasets:
  ✓ data/large/large_1.json - 25 nodes, 5 SCCs of size 5 (edges: 62)
  ✓ data/large/large_2.json - 35 nodes, sparse with cycles (edges: 98)
  ✓ data/large/large_3.json - 45 nodes, large DAG (edges: 210)

All datasets generated successfully!
Verify datasets were created:
bashls data/small/*.json    # Should show 3 files
ls data/medium/*.json   # Should show 3 files
ls data/large/*.json    # Should show 3 files
9.5 Running the Application
Process all datasets in data/ directory:
bashmvn exec:java
This runs the Main class, which processes all JSON files in data/small, data/medium, and data/large.
Process a specific dataset file:
bashmvn exec:java -Dexec.args="data/small/small_1.json"
Run using the compiled JAR:
bashjava -jar target/graph-algorithms-1.0-SNAPSHOT.jar
java -jar target/graph-algorithms-1.0-SNAPSHOT.jar data/medium/medium_1.json
9.6 Expected Output
Successful execution on a dataset produces:
================================================================================
Processing: data/small/small_1.json
================================================================================
Graph: n=6, edges=6, directed=true
0 -> 1(w=3) 
1 -> 2(w=2) 
2 -> 3(w=4) 
3 -> 1(w=1) 
...
SCC 2 (size=1): [4]
SCC 3 (size=1): [5]
Metrics: DFS visits=6, Edges=6, Relaxations=0, Push=0, Pop=0, Time=0.210ms
...
Graph: n=4, edges=1, directed=true
2 -> 3(w=2) 

--- STEP 3: Topological Sort ---
Topological order of components: [2, 1, 0, 3]
Topological order of original tasks: [4, 0, 1, 2, 3, 5]
Metrics: DFS visits=0, Edges=1, Relaxations=0, Push=4, Pop=4, Time=0.120ms

--- STEP 4: Shortest Paths in DAG ---

Shortest Paths from source 2:
To vertex 0: unreachable
To vertex 1: unreachable
To vertex 2: distance=0, path=[2]
To vertex 3: distance=2, path=[2, 3]
Metrics: DFS visits=0, Edges=1, Relaxations=1, Push=0, Pop=0, Time=0.089ms

--- STEP 5: Longest Path (Critical Path) ---
Critical Path: [2, 3], Length: 2

Longest Paths from source 2:
To vertex 0: unreachable
To vertex 1: unreachable
To vertex 2: distance=0, path=[2]
To vertex 3: distance=2, path=[2, 3]
...
Critical Path: [2, 3], Length: 2
Metrics: DFS visits=0, Edges=1, Relaxations=1, Push=0, Pop=0, Time=0.103ms
9.7 Running Specific Tests
Run all tests:
bashmvn test
Run specific test class:
bashmvn test -Dtest=TarjanSCCTest
mvn test -Dtest=TopologicalSortTest
mvn test -Dtest=DAGShortestPathTest
Run with verbose output:
bashmvn test -X
Run specific test method:
bashmvn test -Dtest=TarjanSCCTest#testSimpleCycle
9.8 Troubleshooting
...
Solution: Run DatasetGenerator first to create JSON files

Problem: "Tests failing"

Solution: Run mvn clean test to rebuild from scratch

Problem: "OutOfMemoryError"

Solution: Increase JVM heap size: export MAVEN_OPTS="-Xmx1g"
...
Full clean build: mvn clean package
Run tests: mvn test
Verify JAR: java -jar target/graph-algorithms-1.0-SNAPSHOT.jar
